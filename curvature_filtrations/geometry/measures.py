...
def forman_curvature(G, weight=None) -> np.ndarray:
    """
    Compute the Forman-Ricci curvature for each edge in a graph. The Forman-Ricci curvature is a discrete curvature measure that is defined for edges in a graph.

    We rely on the well maintained `GraphRicciCurvature` package for this computation. The package provides a `FormanRicci` class that can be used to compute the Forman-Ricci curvature for a given graph.

    Parameters
    ----------
    G : networkx.Graph
        The input graph for which the Forman-Ricci curvature is to be computed.
    weight : str, optional
        The edge attribute to be used as weight. If None, all edges are considered to have equal weight.

    Returns
    -------
    np.ndarray
        An array containing the Forman-Ricci curvature values for all edges in the graph.
    """
    forman = FormanRicci(G, weight=weight)
    forman.compute_ricci_curvature()
    curvature_dict = nx.get_edge_attributes(forman.G, name="formanCurvature")
    return np.asarray(list(curvature_dict.values()))


def ollivier_ricci_curvature(G, alpha=0.0, weight=None, **kwargs) -> np.ndarray:
    """
    Compute the Ollivier-Ricci curvature for each edge in a graph.
    ...
    ----------
    G : networkx.Graph
        The input graph on which to compute the Ollivier-Ricci curvature.
    alpha : float, optional
        The parameter alpha for the Ollivier-Ricci curvature, by default 0.0.
    weight : str or None, optional
        The edge attribute to use as weight. If None, all edges are considered to have equal weight, by default None.

    Returns
    -------
    np.ndarray
        An array of Ollivier-Ricci curvature values for each edge in the graph.
    """

    if "prob_fn" in kwargs and kwargs["prob_fn"] is not None:
        print(
            "Custom probability measure detected. Defaulting to custom Ollivier-Ricci curvature."
        )
        return custom_orc(
            G, alpha=alpha, weight=weight, prob_fn=kwargs["prob_fn"]
        )
    else:
        orc = OllivierRicci(G, alpha=alpha, weight=weight)
        orc.compute_ricci_curvature()
        curvature_dict = nx.get_edge_attributes(orc.G, name="ricciCurvature")
        return np.asarray(list(curvature_dict.values()))


def custom_orc(G, alpha=0.0, weight=None, prob_fn=None) -> np.ndarray:
